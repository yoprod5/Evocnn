...
class Individual:
    ...
    def initialize(self,length):
        init_indi = np.random.randint(1,3)
        self.length=length
        if init_indi>=2 :
           self.indi = self.init_with_spaces(length)
        else :
           self.indi =self.init_stand_individual(length)
             

    def init_stand_individual(self,Length):
        print("initializing Inidivdu with number {}...".format(Length))
        _list = []
        i=0
        ...
        while i in range(Length):
            ...
            i+=2
        for _ in range(int(init_num_full)):
            _list.append(self.add_a_random_full_layer())
        _list.append(self.add_a_common_full_layer())
        return _list

#    def init_binar_indiv(self,length):
#        _list = []
#        init_num_full = np.random.randint(1, 3)
#        while i in range(length):
#            init_conv_pool = np.random.randint(1,3)
...
#            _list.append(self.add_a_random_pool_layer())
#            i+=3
#        for _ in range(init_num_full):
#            _list.append(self.add_a_random_full_layer())
#        _list.append(self.add_a_common_full_layer())
#        return _list

    def init_with_spaces(self,length):
        _list = []
        i=0
        init_num_full = np.random.randint(1, 3)
        while i in range(length):
    ...
    def gen_ind_spaces(self, Length):
        print("initializing Inidivdu with number {}...".format(Length))
    ...
    def initialize_spaces(self,length):
        self.length=length
        self.indi = self.gen_ind_spaces(length)
...
#0 add, 1 modify  2delete
    ...
    def add_a_common_full_layer(self):
        mean = self.init_mean()
        std = self.init_std()
        full_layer = FullLayer(hidden_neuron_num=10, weight_matrix=[mean, std])
        return full_layer
