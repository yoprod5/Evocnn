...
class Evaluate:
    ...
    def parse_population_spaces(self, gen_no):
        save_dir = os.getcwd() + '/spaces/save_data/gen_{:03d}'.format(gen_no)
        tf.gfile.MakeDirs(save_dir)
        history_best_score = 0
        for i in range(self.pops.get_pop_size()):
            indi = self.pops.get_individual_at(i)
            ...
            indi.complxity = num_connections
            history_best_score = new_best
            list_save_path = os.getcwd() + '/spaces/save_data/gen_{:03d}/pop.txt'.format(gen_no)
            utils.save_append_individual(str(indi), list_save_path)

        pop_list = self.pops
        list_save_path = os.getcwd() + '/spaces/save_data/gen_{:03d}/pop.dat'.format(gen_no)
        with open(list_save_path, 'wb') as file_handler:
            pickle.dump(pop_list, file_handler)
            ...
            for i in range(num_of_units):
                ...
                if current_unit.type == 1:
                    ...
                    with tf.variable_scope('{}_full_{}'.format(name_preffix, i)):              
                        if i == 0: # use the previous setting to calculate this input dimension 
                           ...
                           #output_list.append(tf.layers.dense(128, activation='relu'))
                           #num_connections += 128
                           #output_list.append(tf.layers.dense(10, activation='softmax'))
                           #num_connections += 10
                           #break
                           last_unit = indi.get_layer_at(i)
                           input_data = slim.flatten(output_list[-1])
                           input_dim = input_data.get_shape()[1].value
                           mean=current_unit.weight_matrix_mean
                           stddev=current_unit.weight_matrix_std
...
#                 optimizer = tf.train.GradientDescentOptimizer(lr)
    ...
    def parse_individual(self, indi, num_of_input_channel, indi_index, save_path, history_best_score,gen_no):
        ...
        #print("x_label shape:",len(train_label.shape))
